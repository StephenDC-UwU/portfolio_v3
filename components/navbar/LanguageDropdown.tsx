    ...
    <div className="absolute right-0 top-full mt-2 py-1.5 w-24 rounded-xl glass-card shadow-lg z-50 text-xs font-cinzel flex flex-col animate-in fade-in zoom-in-95">
      {Object.entries(Languages).map(([key, value]) => (
        <button
          ...
          className={`px-3 py-1.5 text-left hover:bg-black/10 transition-colors flex items-center justify-between cursor-pointer ${
            lang === key ? "font-bold opacity-100" : "opacity-70"
          }`}
        >
          <span>{value}</span>
